...
mod tests {
    ...
    async fn test_stop_request_after_claim_is_not_lost() {
        ...
        // caller has already claimed the scan, and must check the stop signal
        // before opening a raw ICMP socket, so it should see the stop request
        // and exit before scanning any host in the range (and without needing

...
13. **Import locks the vault**, refuses newer schemas, and migrates older ones to the current schema before returning (restoring the previous DB if that fails). Tests: `import_locks_the_vault`, `import_rejects_newer_schema_and_leaves_vault_alone`, `import_migrates_an_older_backup`.

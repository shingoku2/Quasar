...
- **EDW-15: credential writes are now serialized with master-password rekeying.** This was
  ...
  `change_master_password` from just after it sets `changing_password` until the new key is
  installed. A write that arrives mid-rotation now waits and is encrypted under the new key.

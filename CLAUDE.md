...
13. **Import locks the vault**, refuses newer schemas, migrates older ones to the current schema before returning (restoring the previous DB if that fails, and saying so if the restore fails too), and reloads in-memory state loaded from the DB (`AlertEngine::reload` drops all old rules and alert state first; if the new rules can't be read, the import returns an error). Tests: `import_locks_the_vault`, `import_rejects_newer_schema_and_leaves_vault_alone`, `import_migrates_an_older_backup`, `import_restores_the_previous_database_when_migration_fails`, `restore_previous_database_reports_failure`, `import_reloads_alert_rules`, `import_reports_unreadable_alert_rules_and_drops_the_old_ones`, `reload_forgets_the_previous_triggered_state`.

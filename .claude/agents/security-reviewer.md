...
- [ ] **Host key verification**: `known_hosts` checks must not be bypassed. Look for `accept_any` flags, disabled verification callbacks, or `TODO`/`FIXME` comments suppressing verification.
...
- [ ] **Error message leakage**: SSH error messages surfaced to users must pass through `sanitize_error()` — never raw russh errors which may contain session detail.

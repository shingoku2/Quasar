...
- **Local files**: `pick_local_file` / `pick_save_location` open the dialog in Rust and record a single-use grant (read for open, write for save), used up even if the command then fails. SFTP, export, import and scheduled SFTP tasks accept only granted paths.

...
const UpdateBanner: React.FC<UpdateBannerProps> = ({ autoCheck = import.meta.env.PROD }) => {
  const { status, version, installUpdate } = useUpdater(autoCheck);
  const [dismissed, setDismissed] = useState(false);

  if (dismissed || (status !== 'available' && status !== 'downloading')) {
    return null;
  }
